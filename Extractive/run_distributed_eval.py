# Generate synthetic QAs on $dataset_name training contexts based on $TARGET_DEV_QG_CKPT_DIR"
import argparse
import shutil
import time
...
def eval_data_dir(
    ...
    **generate_kwargs,
    ...
    for batch in tqdm(data_loader):
        ...
        #add logits
        logprob=generate_id_to_logits(model, summaries,
        ...
        for i, pred in enumerate(preds):
            results.append(dict(pred=pred, logprob=str(logprob[i]), id=ids[i].item()))
...
def generate_id_to_logits(model, generated_ids, src_ids, src_masks):
    ...
    with torch.no_grad():
        logits = model(src_ids, attention_mask=src_masks,
                              decoder_input_ids=gen_decoder_input_ids, use_cache=False)[0]
...
def run_generate():
    ...
    if args.local_rank <= 0:
        ...
        preds, logprobs = combine_partial_results(partial_results)
        ...
        print(metrics)
        write_txt_file(preds, logprobs, save_dir.joinpath(f"{args.type_path}_generations.txt"))
        if args.debug:
            write_txt_file(labels, logprobs,  save_dir.joinpath(f"{args.type_path}.target"))
        else:
...
def combine_partial_results(partial_results) -> List:
    ...
    scores = [x["logprob"] for x in records]
    return preds,scores
