...
echo "##########################################################################"
  echo "Eval QA model: Source + Target Dev"
  echo "##########################################################################"
  python QA/run_squad.py \
      --model_type bert \
      --model_name_or_path model/QA_NaturalQuestionsShort_Source_TargetDev/ \
      --do_eval \
      --do_lower_case \
      --predict_file data/NaturalQuestionsShort.test.json \
      --per_gpu_train_batch_size 12 \
      --learning_rate 3e-5 \
      --num_train_epochs 2.0 \
      --max_seq_length 384 \
      ...
      --doc_stride 128 \
      --save_steps 20000 \
      --output_dir model/QA_NaturalQuestionsShort_Source_TargetDev